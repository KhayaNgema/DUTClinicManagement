    ...
    public class RegisterNurseModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            try
            {
                ...
                {
                    var profilePicturePath = await _fileUploadService.UploadFileAsync(Input.ProfilePicture);
                    nurse.ProfilePicture = profilePicturePath;
                }

                await _userStore.SetUserNameAsync(nurse, Input.Email, CancellationToken.None);
                await _emailStore.SetEmailAsync(nurse, Input.Email, CancellationToken.None);

                string randomPassword = _passwordGenerator.GenerateRandomPassword();
                var result = await _userManager.CreateAsync(nurse, randomPassword);

                if (result.Succeeded)
                {
                    _logger.LogInformation("Nurse created a new account with password.");

                    await _userManager.AddToRoleAsync(nurse, "Nurse");

                    var code = await _userManager.GenerateEmailConfirmationTokenAsync(nurse);
                    code = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(code));
                    var callbackUrl = Url.Page(
                        "/Account/ConfirmEmail",
                        pageHandler: null,
                        values: new { area = "Identity", userId = nurse.Id, code = code, returnUrl = returnUrl },
                        protocol: Request.Scheme);

                    string accountCreationEmailBody = $"Hello {nurse.FirstName},<br><br>";
                    accountCreationEmailBody += $"Welcome to DUT Clinic!<br><br>";
                    accountCreationEmailBody += $"You have been successfully added as a Nurse. Below are your login credentials:<br><br>";
                    accountCreationEmailBody += $"Email: {nurse.Email}<br>";
                    ...
                    BackgroundJob.Enqueue(() => _emailService.SendEmailAsync(nurse.Email, "Confirm Your Email Address", accountCreationEmailBody, "MediCare"));

                    string emailConfirmationEmailBody = $"Hello {nurse.FirstName},<br><br>";
                    ...
                    BackgroundJob.Enqueue(() => _emailService.SendEmailAsync(nurse.Email, "Confirm Your Email Address", emailConfirmationEmailBody, "Diski 360"));

                    TempData["Message"] = $"{nurse.FirstName} {nurse.LastName} has been successfully added as your new nurse";
                    return RedirectToAction("Nurses", "Users");
                }
            ...
            catch (Exception ex)
            {
                ...
                {
                    success = false,
                    message = "Failed to onboard nurse: " + ex.Message,
                    errorDetails = new
                    {
